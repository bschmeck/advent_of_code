defmodule Mix.Tasks.Fetch do
  ...
  defp published_days() do
    case east_coast_date() do
      %Date{year: 2025, month: 12, day: d} when d <= 25 -> Enum.to_list(1..d)
      %Date{year: 2025, month: 12, day: d} when d > 25 -> Enum.to_list(1..25)
      %Date{year: 2026} -> Enum.to_list(1..25)
      _ -> []
    end
  end
  ...
  defp create_module(day) do
    source = Path.join(:code.priv_dir(:year_2025), "day_module.eex")
    day = day |> Integer.to_string() |> String.pad_leading(2, "0")
    contents = EEx.eval_file(source, day: day)
    location = Path.join([:code.priv_dir(:year_2025), "..", "lib", "day_#{day}.ex"])
    File.write(location, contents)
  end

  defp create_test_file(day) do
    source = Path.join(:code.priv_dir(:year_2025), "day_module_test.eex")
    day = day |> Integer.to_string() |> String.pad_leading(2, "0")
    contents = EEx.eval_file(source, day: day)
    location = Path.join([:code.priv_dir(:year_2025), "..", "test", "day_#{day}_test.exs"])
    File.write(location, contents)
  end

  defp cookie, do: "session=#{Application.fetch_env!(:year_2025, :session)}"
end
