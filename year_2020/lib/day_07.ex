defmodule Day07 do
  ...
  def invert(color, [content | rest], map),
    do: invert(color, rest, Map.put(map, content.color, [color]))
  ...
  defp captures_to_rule(%{"color" => color, "contents" => contents}),
    do: {color, contents |> String.split(",") |> extract_contents()}
  ...
  defp count(rules, bag_color) do
    ...
    1 +
      Enum.reduce(contents, 0, fn content, total ->
        total + content.number * count(rules, content.color)
      end)
  end
end
