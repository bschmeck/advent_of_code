defmodule Day11 do
  ...
  def part_two(input \\ InputFile, size \\ 1_000_000) do
    input.contents_of(11, :stream)
    |> Enum.with_index()
    |> Enum.flat_map(fn {row, y} ->
      row
  ...
  def expand(universe, size \\ 2) do
    ...
    universe
    |> Enum.map(fn {x, y} -> {x + (size - 1) * Enum.count(missing_x, fn x_m -> x_m < x end), y + (size - 1) * Enum.count(missing_y, fn y_m -> y_m < y end)} end)
  end
